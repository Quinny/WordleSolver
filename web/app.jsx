...
class App extends React.Component {
  ...
  focusLeft() {
    this.setState((prevState) => ({
      focusedLetter:
        prevState.focusedLetter == 0 ? 4 : prevState.focusedLetter - 1,
    }));
  }
  ...
  submit() {
    var newConstraints = this.state.constraints.concat(
      getConstraints(
  ...
  render() {
    ...
    var rows = allGuesses.map((guess, guessIndex) => {
      ...
      var values = zipString(guess.guess, guess.verdict).map(
        (letters, index) => {
          var guessLetter = letters[0];
          var verdictLetter = letters[1];
          var extraClass =
            isPendingGuess && index == this.state.focusedLetter
              ? "focused"
              : "";
          return (
            <td
