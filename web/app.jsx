function bestGuess(constraints) {
  var matchesAll = word => constraints.every(constraint => constraint(word));
  return scoredWords.find(matchesAll);
}

function getConstraints(guess, verdict) {
...
function cycleVerdict(verdict, index) {
  var nextMap = {
    'B': 'Y',
    'Y': 'G',
    'G': 'B'
  };
  var next = nextMap[verdict[index]];
  return verdict.substr(0, index) + next + verdict.substr(index + 1);
}
...
class App extends React.Component {
  ...
  focusLeft() {
    this.setState( prevState => ({
      focusedLetter: prevState.focusedLetter == 0 ? 4 : prevState.focusedLetter - 1
    }));
  }

  focusRight() {
    this.setState( prevState => ({
      focusedLetter: (prevState.focusedLetter + 1) % 5
    }));
  }

  focusAndCycleVerdict(index) {
    this.setState(prevState => ({
      pendingGuess: {
        guess: prevState.pendingGuess.guess,
        verdict: cycleVerdict(prevState.pendingGuess.verdict, index)
      },
      focusedLetter: index
    }));
  }
  ...
  submit() {
    var newConstraints = this.state.constraints.concat(getConstraints(this.state.pendingGuess.guess, this.state.pendingGuess.verdict));
    var newGuess = bestGuess(newConstraints);
    this.setState(prevState => ({
  ...
  handleKeyDown(e) {
    // Up
    if (e.keyCode === 38) {
      this.focusAndCycleVerdict(this.state.focusedLetter);
    }
  ...
  render() {
    ...
    var rows = allGuesses.map((guess, guessIndex) => {
      ...
      var values = zipString(guess.guess, guess.verdict)
        .map((letters, index) => {
          var guessLetter = letters[0];
          var verdictLetter = letters[1];
          var extraClass = isPendingGuess && index == this.state.focusedLetter ? "focused" : "";
          return (
            <td
              class={"table-" + verdictClass(verdictLetter) + " " + extraClass}
              onClick={e => this.focusAndCycleVerdict(index)}>
